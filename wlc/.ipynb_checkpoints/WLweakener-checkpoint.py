...
def computeVirtual(z, c, method='IPL', M=None, dec_labels=None, p=None):
    """
    ...
    ----------
    z : list
        Weak labels. Each weak label is an integer whose binary representation
        encodes the observed weak labels

    c : int
        Number of classes. All components of z must be smaller than 2**c

    method : str, optional (default='IPL')
        Method for computation of the virtual label vector v
        Available methods are:
            'supervised'  :Takes virtual labels equal to the binary
                           representations of the weak labels in z
            'IPL'         :Independet Partial Labels. Equivalent to supervised
            'quasi-IPL'   :Virtual labels based on a 'quasi-IPL' mixing matrix
            'known-M-pseudo' :Virtual labels based on the Moore-Penrose
                           pseudo-inverse of M
            'known-M-opt' :Virtual labels minimizing MSE.
            'known-M-opt-conv' :Convexity-preserving virtual labels minimizing
                           MSE
            'MCC'         :Virtual labels for a M-CC loss (not available yet)

    M : numpy.ndarray or None, optional (M=None)
        Mixing matrix. Only for methods 'Mproper' and 'MCC'

    dec_labels : list or None, optional (default=None)
        A list of indices in {0, 1,..., 2**c}: dec_labels[i] is an integer
        whose binary representation encodes the weak labels corresponding to
        the i-th row in M. The length of dec_labels must be equal to the number
        of rows in M
    ...
    if method in ['supervised', 'IPL']:
        ...
        if method == 'known-M-pseudo':
            Y = np.linalg.pinv(M)
        elif method in ['known-M-opt', 'known-M-opt-conv']:
            if p is None:
                binary_z = label_binarize(z, range(2**c))
            else:
                binary_z = np.array([p.squeeze()])
            conv = (method == 'known-M-opt-conv')
            Y = computeVirtualMatrixOptimized(binary_z, M, convex=conv)
        print(Y)
